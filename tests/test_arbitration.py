...
def test_rule0_gate_queues_speech_but_allows_short_ack_in_full_duplex():
    q, player, _ = make()
    q.set_gate(True)
    # Real timeline: chirp at VAD close (gated window), mate speech later.
    q.enqueue(ack())
    assert [i.source for i in player.played] == [Source.ACK]  # exemption
    q.on_item_finished()
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "hello", turn_id="t-1"))
    assert len(player.played) == 1  # mate speech still gated
    q.set_gate(False)
    assert [i.source for i in player.played] == [Source.ACK, Source.FIRST_MATE]
...
def test_rule1_barge_in_flushes_playing_and_queued():
    ...
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "a", turn_id="t-1"))
...
def test_rule2_current_turn_agent_say_preempts_first_mate():
    q, player, _ = make()
    q.note_dispatch("t-7")
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "thinking out loud", turn_id="t-7"))
    assert player.played[-1].source is Source.FIRST_MATE
...
def test_rule2_old_turn_agent_say_queues_behind():
    q, player, _ = make()
    q.note_dispatch("t-9")
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "current turn speech", turn_id="t-9"))
    q.enqueue(PlaybackItem(Source.AGENT, "late say from before", turn_id="t-2"))
    assert player.stops == 0  # no preemption
    assert player.played[-1].source is Source.FIRST_MATE


def test_rule3_first_mate_never_plays_after_agent_spoke_that_turn():
    q, player, _ = make()
    q.note_dispatch("t-3")
    q.enqueue(PlaybackItem(Source.AGENT, "answer", turn_id="t-3"))
    q.on_item_finished()
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "stale ack", turn_id="t-3"))
    assert [i.source for i in player.played] == [Source.AGENT]


def test_rule4_fillers_dropped_when_real_speech_exists():
    q, player, _ = make()
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "speaking", turn_id="t-1"))
    q.enqueue(ack())
    q.on_item_finished()
    assert [i.source for i in player.played] == [Source.FIRST_MATE]
    ...
    q.set_gate(False)
    assert [i.source for i in player.played] == [Source.FIRST_MATE, Source.AGENT]


def test_rule5_chime_waits_for_idle():
    q, player, _ = make()
    q.enqueue(PlaybackItem(Source.FIRST_MATE, "talking", turn_id="t-1"))
